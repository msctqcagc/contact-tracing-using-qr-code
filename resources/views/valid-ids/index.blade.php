...
<div class="row">
    <div class="col-lg-12">
        <div class="breadcrumb-main">
            <h4 class="text-capitalize breadcrumb-title">Valid ID's</h4>
...
<div class="row">
    <div class="col-lg-12 mb-30">
        <div class="card">
            ...
            <div class="card-body">
                <table id="tableValidIDs" class="stripe" style="width:100%">
                    <thead>
                        <tr>
                            <th>ABBR</th>
                            <th>Name</th>
...
<script>
    ...
    function fetchValidIDs() {
        $("#tableValidIDs").DataTable().rows().clear().draw();

        axios.get('/api/valid-ids')
            ...
            for(let i = 0; i < valid_ids.length; i++) {
                ...
                <div class="atbd-button-list d-flex flex-wrap" style="float:right">
                    <button class="btn btn-icon btn-squared btn-outline-warning btn-update" data-toggle="tooltip" data-placement="left" title="Edit"
                        data-id="` + valid_id.id + `"
                        data-abbr="` + valid_id.abbr + `"
                        data-name="` + valid_id.name + `"
                        data-description="` + valid_id.description + `">
                    ...
                    <button class="btn btn-icon btn-squared btn-outline-danger btn-delete" data-id="` + valid_id.id + `" data-toggle="modal" data-target="#modalDelete" data-toggle="tooltip" data-placement="left" title="Delete">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash nav-icon"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                    </button>
                </div>`;

                let created_at = valid_id.created_at;
                if (created_at === '0 seconds ago') {
                    created_at = 'Just Now';
                }

                let updated_at = valid_id.updated_at;
                if (updated_at === '0 seconds ago') {
                    updated_at = 'Just Now';
                }

                $('#tableValidIDs').dataTable().fnAddData([
                    valid_id.abbr,
                    valid_id.name,
                    is_active,
                    created_at,
