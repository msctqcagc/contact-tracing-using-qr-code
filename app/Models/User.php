...
class User extends Authenticatable
{
    ...
    public function filePath() {
        if (!$this->attributes['profile_photo_path']) {
            if (strtolower($this->role->name) === 'administrator') {
                return asset('template/img/default/admin.png');
            }
        } else {
            return asset('uploads/images/profiles/' . $this->attributes['profile_photo_path']);
        }
    }
    ...
    public function scanner() {
        return $this->hasOne(Scanner::class, 'user_id');
    }
